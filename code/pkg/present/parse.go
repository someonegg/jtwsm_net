...
// sub-templates.
func renderElem(t *template.Template, ep ElemProxy) (template.HTML, error) {
	e := ep.Inner.(Elem)
...
func (ctx *Context) Parse(r io.Reader, name string, mode ParseMode) (*Doc, error) {
	...
	// Sections
	if secs, err := parseSections(ctx, name, lines, []int{}, doc); err != nil {
		return nil, err
	} else {
		for _, s := range secs {
			doc.Sections = append(doc.Sections, ElemProxy{s, s.TemplateName()})
		}
	}
	return doc, nil
}
	...
	for i := 1; ; i++ {
		...
		for ok && !lesserHeading(text, prefix) {
			...
			case unicode.IsSpace(r):
				...
				e = &Text{Lines: []string{pre}, Pre: true, Code: isCode}
			...
			case strings.HasPrefix(text, prefix+"* "):
				...
				for _, ss := range subsecs {
					section.Elems = append(section.Elems, ElemProxy{ss, ss.TemplateName()})
				}
			case strings.HasPrefix(text, "."):
			...
			if e != nil {
				section.Elems = append(section.Elems, ElemProxy{e, e.TemplateName()})
			}
			text, ok = lines.nextNonEmpty()
		}
...
func parseHeader(doc *Doc, lines *Lines) error {
	...
	for {
		...
		const tagPrefix, orgURLPrefix = "Tags:", "OriginalURL:"
		...
		} else if strings.HasPrefix(text, orgURLPrefix) {
			u, err := url.Parse(strings.TrimSpace(text[len(orgURLPrefix):]))
			if err != nil {
				return err
			}
			doc.OrgURL = u
...
func parseAuthors(lines *Lines) (authors []Author, err error) {
	...
	for {
		...
		if a != nil && len(text) == 0 {
			if len(a.Name) == 0 {
				log.Printf("Author information is incomplete: no name")
			} else {
		...
		case strings.Contains(text, ":"):
			if u := parseURL(text); u != nil {
				a.URL = u
			}
		case strings.Contains(text, "@"):
			if u := parseURL("mailto:" + text); u != nil {
				a.Email = u
			}
		default:
			if len(a.Name) == 0 {
				...
				log.Printf("Author text(%q) discard", text)
			}
		}
	}
	if a != nil {
...
func init() {
	RegisterUnmarshaler("section", unmarshalSection)
	RegisterUnmarshaler("text", unmarshalText)
	RegisterUnmarshaler("list", unmarshalList)
}

func unmarshalSection(data []byte, worker UnmarshalWork) (Elem, error) {
	var ep struct {
		Inner *Section
		Type  string
	}
...
func unmarshalText(data []byte, worker UnmarshalWork) (Elem, error) {
	var ep struct {
		Inner *Text
		Type  string
	}
...
func unmarshalList(data []byte, worker UnmarshalWork) (Elem, error) {
	var ep struct {
		Inner *List
		Type  string
	}
